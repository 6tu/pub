...
dns_he_add() {
  ...
  if [ "$exit_code" -eq 0 ]; then
    _info "The record added successfully."
  else
    _err "Couldn't add the record."
  fi
  _debug2 response "$response"
  return "$exit_code"
}

#-- dns_he_rm() - Remove the record ------------------------------------
...
dns_he_rm() {
  ...
  if ! _contains "$response" "$_txt_value"; then
    _debug "The record is not found, just skip"
    return 0
  fi
